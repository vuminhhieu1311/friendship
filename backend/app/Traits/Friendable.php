...
trait Friendable
{
    ...
    {
        $friends = array();
        $f1 = Friend::where('status', config('request.accepted'))
        ...
        $friends2 = array();
        $f2 = Friend::where('status', config('request.accepted'))
    ...
    {
        $users = array();
        $friendships = Friend::where('status', config('request.pending'))
            ->where('user_requested', $this->id)
            ->get();
    ...
    public function accept_friend($requester)
    {
        ...
        if ($friendship) {
            $friendship->update([
                'status' => config('request.accepted')
            ]);
